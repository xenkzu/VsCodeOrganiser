...
export function extractSignals(document: vscode.TextDocument): FileSignal {
  ...
  const sentinelMatches: string[] = SENTINELS.filter(sentinel =>
    new RegExp(`\\b${sentinel}\\b`, 'i').test(rawSnippet)
  );
  ...
  if (internalLang === 'python') {
    ...
    extractedVars = matchAllFromExactPattern(
      text, /(?:int|long|boolean|String|List|Map|Set|Queue|Stack|Deque|Node|TreeNode|ListNode)\s+(\w+)\s*[=;(,]/g, 1
    );
  } else if (internalLang === 'cpp') {
    extractedVars = matchAllFromExactPattern(
      text, /(?:int|long|bool|string|vector|map|set|queue|stack|deque|auto)\s+(\w+)\s*[=;({,]/g, 1
    );
  } else {
    // TypeScript / JavaScript
    extractedVars = matchAllFromExactPattern(
      text, /(?:const|let|var)\s+(\w+)/g, 1
    );
  }
  ...
  const earlyExitSignal = [...classList, ...methodList]
    .some(name => HIGH_CONFIDENCE_NAMES.some(h =>
...
function extractComments(snippet: string, language: Language): string[] {
