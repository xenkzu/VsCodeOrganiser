...
function scrubSecrets(code: string): string {
  return code
    ...
    .replace(/\b[0-9a-f]{32,64}\b/gi, (match) =>
      match.length >= 32 ? '[HEX_REDACTED]' : match
    );
}
...
export async function classifyAI(
  ...
  if (parsedUrl.protocol !== 'https:') {
    outputChannel.appendLine(
      'AI classifier skipped: endpoint must use HTTPS'
    );
    return null;
  }
  ...
  if (isBlocked) {
    outputChannel.appendLine(
      'AI classifier skipped: endpoint resolves to a private/internal address'
    );
    return null;
  }
